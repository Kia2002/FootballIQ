...
**The ambiguity problem and how it's solved:** many players share a name (there are multiple "David Silva"s in football history). Matching by name alone risks silently assigning the wrong birth date. The fix: match by **name + known club** together. Every ingested player already has a club via `PlayerSeasonStats → Club`, so this disambiguator is free — no new data needed. A name match is only accepted if at least one of Wikidata's recorded clubs for that candidate matches the player's known club; if zero or more than one candidate satisfies that, the player is skipped rather than guessed at.
